...
class SalesDashboard(QWidget):
    ...
    def _submit_invoice(self):
        ...
        if mode == "عميل":
            ...
            # Build elegant thermal text receipt for products
            header = [
                "==============================",
                "   صالون مينا العربي",
            ...
            table_header = f"{'المنتج':<18}{'كمية':>5}{'سعر':>8}{'الإجمالي':>10}"
            rows_txt = [table_header, "-" * len(table_header)]
            for _, name, price, qty in items:
                rows_txt.append(f"{name:<18}{qty:>5}{int(round(price)):>8}{int(round(price*qty)):>10}")
            totals = [
                "------------------------------",
                f"الإجمالي قبل الخصم: {format_amount(total)} ج.م",
                f"الخصم: {discount_percent}%",
                f"الإجمالي بعد الخصم: {format_amount(total * (1 - discount_percent/100.0))} ج.م",
                "------------------------------",
                "",
            ...
            receipt_text = "\n".join(header + rows_txt + totals)

            # Save text and print via thermal printer
            write_ok = False
            try:
                with open(txt_path, "w", encoding="utf-8") as ftxt:
                    ftxt.write(receipt_text); ftxt.flush()
